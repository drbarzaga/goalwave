...
function TooltipOverlay() {
  ...
  // Update arrowElement when arrowRef.current changes (after initial render)
  React.useEffect(() => {
  ...
  // Memoize middleware to avoid reading ref during render
  const middleware = React.useMemo(
    () => [
      ...
      shift({ padding: 8 }),
      floatingArrow({
        element: arrowElement ? { current: arrowElement } : null,
      }),
    ],
    [rendered.data?.sideOffset, rendered.data?.alignOffset, arrowElement]
  );
  ...
  // Use state to store floating element and set it in useEffect
  const [floatingElement, setFloatingElement] =
    React.useState<HTMLDivElement | null>(null);

  // Set floating element in refs after render
  React.useEffect(() => {
    if (floatingElement) {
  ...
  // Callback ref to update floating element state
  const floatingRef = React.useCallback((element: HTMLDivElement | null) => {
