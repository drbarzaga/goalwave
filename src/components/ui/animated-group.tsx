...
function AnimatedGroup({
  ...
  // Store components in state - initialized lazily, created in useEffect
  const [MotionComponent, setMotionComponent] =
    React.useState<React.ElementType | null>(null);
  const [MotionChild, setMotionChild] =
    React.useState<React.ElementType | null>(null);

  // Create motion components in useEffect (after render, not during)
  React.useEffect(() => {
    // This runs after render, so motion.create() is safe here
    setMotionComponent(
      getOrCreateMotionComponent(as as string | React.ComponentType)
    );
    setMotionChild(
      getOrCreateMotionComponent(asChild as string | React.ComponentType)
    );
  }, [as, asChild]);

  // Fallback to original components if motion components are not ready yet
  if (!MotionComponent || !MotionChild) {
    const ContainerComponent = as as React.ElementType;
    const ChildComponent = asChild as React.ElementType;
    return (
